...
<body>
    ...
    <form id="empSearchForm" action="${pageContext.request.contextPath}/Statistics/paymentStatisticsMonthEmployeeModal.do" method="GET" onsubmit="return validateSearch()">
        <div class="search-bar">
            <div class="search-input-group">
                <input type="text" id="empNameInput" name="empName" placeholder="사원검색" style="width: 120px;" value="${empName}">
                <button type="submit" class="btn-search">🔍</button>
                <button type="button" class="btn-view-all" onclick="location.href='${pageContext.request.contextPath}/Statistics/paymentStatisticsMonthEmployeeModal.do'">전체보기</button>
            </div>
    ...
    <div style="text-align: center; margin-top: 15px;">
        <c:if test="${currentPage > 1}">
            <a href="${pageContext.request.contextPath}/Statistics/paymentStatisticsMonthEmployeeModal.do?page=${currentPage - 1}&empName=${empName}&department=${selectedDept}&status=${selectedStatus}" style="text-decoration:none; color:black; margin-right:10px;">◀ 이전</a>
        </c:if>
        ...
        <c:forEach begin="1" end="${totalPage}" var="pageNum">
            <c:choose>
                ...
                <c:otherwise>
                    <a href="${pageContext.request.contextPath}/Statistics/paymentStatisticsMonthEmployeeModal.do?page=${pageNum}&empName=${empName}&department=${selectedDept}&status=${selectedStatus}" style="text-decoration:none; color:black; margin: 0 5px;">${pageNum}</a>
                </c:otherwise>
            </c:choose>
        </c:forEach>

        <c:if test="${currentPage < totalPage}">
            <a href="${pageContext.request.contextPath}/Statistics/paymentStatisticsMonthEmployeeModal.do?page=${currentPage + 1}&empName=${empName}&department=${selectedDept}&status=${selectedStatus}" style="text-decoration:none; color:black; margin-left:10px;">다음 ▶</a>
        </c:if>
    </div>
    ...
    <script>
        ...
        function confirmSelection() {
            ...
            if (window.opener && !window.opener.closed) {
                window.opener.pstmSetSelectedEmployee(selectedName);
                window.close();
            } else {
