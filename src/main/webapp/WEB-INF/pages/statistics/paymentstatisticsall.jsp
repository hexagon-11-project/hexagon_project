...
<%@ page import="statistics.model.AnnualTotalStatistics"%>
...
for (int i = 0; i < annualTotalList.size(); i++) {
	AnnualTotalStatistics row = annualTotalList.get(i);
	if (i > 0) {
		labelsJson.append(',');
...
int selectedEndYear = request.getAttribute("endYear") == null
		? java.time.LocalDate.now().getYear()
		: (Integer) request.getAttribute("endYear");
...
<section class="card chart-card">
	...
	<div class="card-body">
		...
		<div class="chart-canvas-wrap">
			<canvas id="paymentStatisticsAllCanvas" data-chart="combo"
				data-source="paymentStatisticsAllChartData"></canvas>
			<div id="paymentStatisticsAllTooltip" class="chart-tooltip" hidden></div>
		</div>
		<script>
		(function () {
			var hitAreas = [];
			...
			function drawCombo() {
				var canvas = document.getElementById('paymentStatisticsAllCanvas');
				var data = window.paymentStatisticsAllChartData;
				...
				for (var b = 0; b < barValues.length; b++) {
					...
					hitAreas.push({
						...
						lineName: lineName,
						barValue: v,
						lineValue: Math.round((lineValues[b] || 0) * 10) / 10
					});
				...
				for (var m = 0; m < lineValues.length; m++) {
					...
					ctx.textAlign = 'center';
					ctx.textBaseline = 'bottom';
					ctx.fillText(String(Math.round(lineValues[m] * 10) / 10), mx, my - 8);
				}
